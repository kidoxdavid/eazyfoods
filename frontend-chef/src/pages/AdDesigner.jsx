...
function StripePaymentForm({ paymentIntentId, onConfirmRef }) {
  ...
  const confirmFn = useCallback(async () => {
    ...
    const { error } = await stripe.confirmPayment({
      elements,
      confirmParams: { return_url: window.location.origin + '/ads' },
    })
...
const AdDesigner = () => {
  ...
  useEffect(() => {
    ...
    api.post('/chef/marketing/create-ad-payment-intent', { amount: selectedDuration.cost })
      .then(res => {
      ...
      .catch(() => { if (!cancelled) setStripeClientSecret('') })
