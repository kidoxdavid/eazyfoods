        ...
        <thead className="bg-gray-50">
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className={`px-4 xl:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase ${
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {sortedData.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-gray-50">
              {columns.map((column) => (
                <td key={column.key} className="px-4 xl:px-6 py-4 whitespace-nowrap text-sm">
