...
const AdDesigner = () => {
  ...
  const fetchAd = async () => {
    try {
      ...
      const response = await api.get(`/vendor/marketing/ads/${id}`)
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      // Store the URL as returned (relative path)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/vendor/marketing/ads/${id}`, data)
      } else {
        await api.post('/vendor/marketing/ads', data)
      }
  ...
  return (
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      <div className="flex items-center justify-between">
        <Link to="/ads" className="text-primary-600 hover:text-primary-700 flex items-center gap-2 text-sm sm:text-base">
      ...
      <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
        <div className="lg:col-span-2 space-y-4 sm:space-y-5 lg:space-y-6">
          <div className="bg-white rounded-lg shadow border border-gray-200 p-4 sm:p-5 lg:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">Ad Details</h2>
            
            <div className="space-y-3 sm:space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Ad Name *</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Ad Type *</label>
                  <select
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Placement *</label>
                  <select
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Upload Image</label>
                <div>
                  <label className="flex items-center justify-center w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-primary-500 hover:bg-primary-50 transition-colors">
                    <div className="flex flex-col items-center">
                      <Upload className="h-4 w-4 sm:h-5 sm:w-5 text-gray-400 mb-1" />
                      <span className="text-xs sm:text-sm text-gray-600">
                        {uploadingImage ? 'Uploading...' : 'Click to Upload Image'}
                      </span>
                      <span className="text-[10px] sm:text-xs text-gray-500 mt-1">JPEG, PNG, WebP, GIF (max 10MB)</span>
                    </div>
                    <input
                      type="file"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                  Image URL <span className="text-gray-400 font-normal text-[10px] sm:text-xs">(Optional - use if you have an external image URL)</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg"
                  placeholder="https://example.com/image.jpg (optional - leave empty if you uploaded an image above)"
                />
                <p className="mt-1 text-[10px] sm:text-xs text-gray-500">Leave empty if you uploaded an image above</p>
              </div>

              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Video (Optional)</label>
                <div className="space-y-3">
                  {/* File Upload */}
                  <div>
                    <label className="flex items-center justify-center w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-primary-500 hover:bg-primary-50 transition-colors">
                      <div className="flex flex-col items-center">
                        <Video className="h-4 w-4 sm:h-5 sm:w-5 text-gray-400 mb-1" />
                        <span className="text-xs sm:text-sm text-gray-600">
                          {uploadingVideo ? 'Uploading...' : 'Upload Video'}
                        </span>
                        <span className="text-[10px] sm:text-xs text-gray-500 mt-1">MP4, WebM, OGG, QuickTime (max 50MB)</span>
                      </div>
                      <input
                        type="file"
                ...
                {formData.video_url && (
                  <div className="mt-3">
                    <video
                      ...
                      onError={(e) => {
                        console.error('Video failed to load:', formData.video_url)
                        e.target.style.display = 'none'
                      }}
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">CTA Text</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">CTA Link</label>
                  <input
                    type="url"
          ...
          {/* Schedule & Slideshow Settings */}
          <div className="bg-white rounded-lg shadow border border-gray-200 p-4 sm:p-5 lg:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">Schedule & Display Settings</h2>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg"
                  />
                  <p className="mt-1 text-[10px] sm:text-xs text-gray-500">When should this ad start showing?</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg"
                  />
                  <p className="mt-1 text-[10px] sm:text-xs text-gray-500">When should this ad stop showing?</p>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="0"
                />
                <p className="mt-1 text-[10px] sm:text-xs text-gray-500">Higher priority ads show first (0-100)</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow border border-gray-200 p-4 sm:p-5 lg:p-6 lg:sticky lg:top-6">
            ...
            >
              {formData.video_url && (
                <video
                  ...
                  onError={(e) => {
                    console.error('Preview video failed to load:', formData.video_url)
                  }}
                >
                  Your browser does not support the video tag.
                </video>
